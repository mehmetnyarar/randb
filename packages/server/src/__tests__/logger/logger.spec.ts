...
describe('logger', () => {
  ...
  describe('store', () => {
    it('should skip (silent)', () => {
      const logger = Logger.create({ level: 'off', file: 'info' })
    ...
    describe('db', () => {
      it('should skip (>level)', () => {
        const logger = Logger.create({ db: 'error' })
        const storage = logger.storages[0]
        const spy = jest.spyOn(storage, 'fn')
      ...
      it('should throw an error (not implemented)', () => {
        const logger = Logger.create({ db: 'info' })
        const storage = logger.storages[0]
        const spy = jest.spyOn(storage, 'fn')
    ...
    describe('file', () => {
      it('should skip (>level)', () => {
        const logger = Logger.create({ file: 'error' })
        const storage = logger.storages[0]
        const spy = jest.spyOn(storage, 'fn')
      ...
      it('should store', () => {
        const logger = Logger.create({ src: 'test', file: 'info' })
        const storage = logger.storages[0]
        const spy = jest.spyOn(storage, 'fn')
  ...
  describe('create', () => {
    ...
    it('should create a logger with file storage', () => {
      const logger = Logger.create({ file: 'info' })
      expect(logger.storages).toHaveLength(1)
    })
